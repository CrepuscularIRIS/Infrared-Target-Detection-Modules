...
def bbox_iou(
    ...
    Args:
        ...
        GaussianIoU (bool, optional): If True, calculate Gaussian IoU for small target detection.
        eps (float, optional): A small value to avoid division by zero.
        k (float, optional): Scale factor for Gaussian IoU sigma calculation. Default 0.5.
        lam (float, optional): Weight factor for Gaussian center loss term. Default 1.0.

    Returns:
        (torch.Tensor): IoU, GIoU, DIoU, CIoU, or GaussianIoU values depending on the specified flags.
    """
    # Get the coordinates of bounding boxes
    ...
    iou = inter / union
    if GaussianIoU:
        # Gaussian IoU: L = L_CIoU + λ · L_center
        # First calculate CIoU as base IoU loss
        ...
        # Gaussian center loss term
        # Center coordinates of predicted and ground truth boxes
        if xywh:
            xp, yp = x1, y1  # predicted center
            xg, yg = x2, y2  # ground truth center
            wg, hg = w2, h2  # ground truth width and height
        else:
            xp = (b1_x1 + b1_x2) / 2  # predicted center x
            yp = (b1_y1 + b1_y2) / 2  # predicted center y
            xg = (b2_x1 + b2_x2) / 2  # ground truth center x
            yg = (b2_y1 + b2_y2) / 2  # ground truth center y
            wg, hg = w2, h2  # ground truth width and height
        
        # Center distance squared
        d2 = (xp - xg).pow(2) + (yp - yg).pow(2)
        
        # Scale-adaptive sigma: σ = k * √(wg * hg)
        sigma = k * (wg * hg).sqrt()
        sigma2 = sigma.pow(2)
        
        # Gaussian weight: w = exp(-d²/(2σ²))
        gaussian_weight = torch.exp(-d2 / (2 * sigma2 + eps))
        
        # Smoother, bounded center loss: L_center = 1 - w
        center_loss = 1 - gaussian_weight
        
        # Final Gaussian IoU loss: L = L_CIoU + λ * L_center
        # Note: We return 1 - loss to maintain IoU format (higher is better)
        gaussian_loss = (1 - ciou) + lam * center_loss
        return 1 - gaussian_loss
